...
public class Queries_Facturas {
    ...
    public void buscar(JTable tabla){
        ...
        String [] columnas={"ID Factura","Cliente","Empleado", "Producto","Cantidad","Precio","Tipo Pago"};
    ...
    public void modificar(String codigo, String cliente, String empleado,String producto, String cantidad, String precio, String tipoPago){
        conn.conexion();
        try{
            conn.conexion();
            Statement query = conn.conexion().createStatement();
            query.execute("update factura set idCliente = '"+cliente+"', idEmpleado = '"+empleado+
                   "', idProducto='"+producto+"', cantidad='"+cantidad+"', precio='"+precio+"', tipoPago='"+tipoPago+"' where idFactura="+codigo);
